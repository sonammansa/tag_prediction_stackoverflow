...
for x in results:
    ...
    cursor.execute("SELECT score from posts where userid="+(str)(x[4]))
    res1 = cursor.fetchall()
    for r1 in res1:
        scr=scr+r1[0]
    t.append(scr)  #score of other posts of the user
    t1=0
    d=0
...
cursor.execute("update posts_test set d_closed1=0")
db.commit()
...
for line1 in results1:
    ...
    if m==0:
        t.append(m)
        t.append(m)
    cursor.execute("SELECT score from posts where userid="+(str)(line1[5]))
    res1 = cursor.fetchall()
    for r1 in res1:
        scr=scr+r1[0]
    t.append(scr)  #score of other posts of the user
    ...
    l= rf.predict(t)
    if l[0]==1:
        cursor.execute("update posts_test set d_closed1=1 where id="+(str)(line1[0]))
        db.commit()

cursor.execute("SELECT count(*) from posts_test where closed=d_closed1")
(result,) = cursor.fetchone()
ac=Decimal(Decimal(result)/Decimal(1000))
print ("accuracy: %f" % ac)
cursor.execute("update c_accuracy set d_accuracy="+(str)(ac)+" where sno=1")
db.commit()
...
#X = [[1,1,1], [2,2,2],[3,3,3]]
#Y = [4,5,6]
#clf = DecisionTreeClassifier(n_estimators=10)
#clf = clf.fit(X, Y)
#a=[[2,3,1],[3,4,5]]
#print clf.predict(a)
