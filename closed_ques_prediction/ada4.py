...
cursor.execute("update posts_test set a_closed4=0")
db.commit()
...
for line1 in results1:
    ...
    l= rf.predict(t)
    if l[0]==1:
        cursor.execute("update posts_test set a_closed4=1 where id="+(str)(line1[0]))
        db.commit()

cursor.execute("SELECT count(*) from posts_test where closed=a_closed4 and id<=27952") # id 27952 for 1000test, 23376 for 500 and so on
(result,) = cursor.fetchone()
ac=Decimal(Decimal(result)/Decimal(1000)) #divide by 1000.0 for 1000test, 990.0 for 990 test, and so on
print ("accuracy: %f" % ac)
cursor.execute("update c_accuracy1 set a_accuracy="+(str)(ac)+" where sno=1") # sno 1 for 1000:1000, and so on
db.commit()
