...
rf = RandomForestClassifier(n_estimators=15)   # change no of estimators accordingly
rf = rf.fit(train, target)

cursor.execute("update posts_test set r_closed2=0")
db.commit()
...
for line1 in results1:
    ...
    l= rf.predict(t)
    if l[0]==1:
        cursor.execute("update posts_test set r_closed2=1 where id="+(str)(line1[0]))
        db.commit()

cursor.execute("SELECT count(*) from posts_test where closed=r_closed2")
(result,) = cursor.fetchone()
ac=Decimal(Decimal(result)/Decimal(1000))
print ("accuracy: %f" % ac)
cursor.execute("update c_accuracy set accuracy="+(str)(ac)+" where sno=5")    #change sno accordingly
db.commit()
