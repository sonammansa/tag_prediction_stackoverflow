...
cursor.execute("SELECT score,title,body,closed,userid from posts limit 2000") #limit 1000 for 1000 train, and so on
...
cursor.execute("update posts_test set r_closed4=0")
db.commit()

cursor.execute("SELECT id,score,title,body,closed,userid from posts_test limit 500") #limit 1000 for 1000 test, and so on
...
for line1 in results1:
    ...
    l= rf.predict(t)
    if l[0]==1:
        cursor.execute("update posts_test set r_closed4=1 where id="+(str)(line1[0]))
        db.commit()

cursor.execute("SELECT count(*) from posts_test where closed=r_closed4 and id<=23376") # id 27952 for 1000test, 23376 for 500 and so on
(result,) = cursor.fetchone()
ac=Decimal(Decimal(result)/Decimal(500)) #divide by 1000.0 for 1000test, 990.0 for 990 test, and so on
print ("accuracy: %f" % ac)
cursor.execute("update c_accuracy1 set accuracy="+(str)(ac)+" where sno=6") # sno 1 for 1000:1000, and so on
db.commit()
